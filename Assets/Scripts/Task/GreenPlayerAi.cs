...
public class GreenPlayerAi : MonoBehaviour
{
    [SerializeField]
    float speed = 20f;
    ...
    void OnEnable()
    {
        ...
        SpawnManager.greenPlayerCount += 1;
    }
        ...
        while (true)
        {
            foreach (Vector3 dir in directions)
            {
                while (!Physics.Raycast(transform.position, dir, out RaycastHit hitinfo, 10f))
                {
                    ...
                    {
                        if (hit.collider.CompareTag("RedPlayer") || hit.collider.CompareTag("BluePlayer") && !isShoot)
                        {
